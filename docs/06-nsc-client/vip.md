...
| tun | `10.100.0.0` | `/16` | RFC1918 私有网段，内核路由到 `tun0`；NSC 和 NSGW 侧双向使用同一段 |

定义见 `crates/nsc/src/vip.rs:19`：

```rust
pub fn new_userspace() -> Self { Self::with_prefix([127, 11]) }
pub fn new_tun() -> Self       { Self::with_prefix([10, 100]) }
```

### 为什么选 `127.11`？

- `127.0.0.0/8` 整段在 Linux/BSD/macOS 上默认可用于环回，无须单独 `ifconfig alias`。
- `127.0.0.1` 是默认 localhost，`127.0.0.53` 被 systemd-resolved 惯用，避免冲突。
...
- `10.100.0.0/16` 在 RFC1918 范围内，便于在 `ip route add 10.100.0.0/16 dev tun0` 之后被内核路由。
- 和 NSGW 的 WG 隧道子网协调一致（见 `/app/ai/nsio/docs/architecture.md`）。
...
- `userspace_prefix_is_127_11` / `tun_prefix_is_10_100` — 段正确；
...
sequenceDiagram
  autonumber
  participant NSD
  participant NSC as NSC main loop
  participant A as VipAllocator
  participant R as NscRouter
  participant D as DnsRecords
  participant P as ProxyManager

  NSD->>NSC: SSE routing_config (routes[])
  loop for each route
    NSC->>A: allocate(route.site)
    A-->>NSC: vip (127.11.x.x)
    NSC->>R: update_routing → insert sites_by_name/vip + routes
    NSC->>D: insert(route.domain.lower → vip)
  end
  NSC->>P: proxy_mgr.update()
  P->>P: 对新 (VIP, port) 启动 TCP listener
  Note over NSC,P: 进程退出 → 所有状态丢失 (ephemeral)
```
...
3. **TUN 模式下的路由冲突**：若用户本机已有 `10.100.0.0/16` 的静态路由（VPN 等），NSC 的 TUN 设备会抢占。这种场景目前无显式检测，需通过 `--data-plane userspace` 回退。
