...
- **[01 系统总览](../01-overview/README.md)** — NSC 是 NSIO 四大组件之一（NSD / NSGW / NSN / NSC），用户侧入口。
- **[02 控制面](../02-control-plane/README.md)** — NSC 复用 `ControlPlane`，通过 SSE 订阅 `routing_config` / `dns_config` / `gateway_config`。
- **[03 数据面](../03-data-plane/README.md)** — NSC `proxy.rs` 和 NSN 共用相同的 WsFrame 协议（`CMD_OPEN_V4` / `CMD_DATA` / `CMD_CLOSE`）；TUN 模式复用 `tunnel-wg`。
- **[05 代理与 ACL](../05-proxy-acl/README.md)** — NSC 的 proxy 是**出站方向**，与 NSN 的入站代理方向相反；ACL 过滤待实现（见 design.md 的限制章节）。
- **[07 NSN 节点](../07-nsn-node/README.md)** — NSC 和 NSN 都是 Rust 二进制，共享控制面/鉴权/隧道栈，但流量方向相反，参考 design.md 末尾的对照表。
...
flowchart LR
  ...
  OS --> DNS["NSC DNS 127.0.0.53:53<br/>(dns.rs)"]
