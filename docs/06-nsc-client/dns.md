...
- NSC DNS 是**必经路径**。`/etc/resolv.conf` 里需要把 `nameserver 127.53.53.53`(默认监听地址)放在最前,或用 `resolvectl domain <iface> '~n.ns'` 让 systemd-resolved 把 `*.n.ns` 转发给 NSC。NSC 本身**不修改**系统配置,由用户或部署脚本负责。
...
### 为什么选 `127.53.53.53:53`

1. **避开所有已知 stub resolver 段**。标准占用都集中在 `127.0.0.x`:
   - `127.0.0.1:53` — Linux/macOS 传统 localhost DNS;macOS mDNSResponder 偶尔绑这里
   - `127.0.0.53:53` — **systemd-resolved 的 stub**(Ubuntu 18.04+ / Debian 12 / Fedora / Arch 默认占着)
   - `127.0.0.2:53` / `127.0.0.254:53` 等 — dnsmasq / Pi-hole / 各种 resolver 常见 alias
   - `127.0.0.1:5353` — mDNS (Bonjour/Avahi) 惯用端口

   `127.53.53.53` 彻底离开 `127.0.0.0/24`,**没有任何主流软件**默认使用这个地址。
2. **自文档化的 mnemonic**。四个 53 一眼就是"本地 DNS"(端口 53 = DNS 协议);和 `1.1.1.1` / `8.8.8.8` / `9.9.9.9` 这类易记公共 resolver 是同一种命名美学。
3. **仍在 `127.0.0.0/8`**,Linux 内核默认把整段 routes 到 `lo`,无须 `ip addr add`。端口 53 让 `/etc/resolv.conf` 零改写(不需要写 `:port`)。
...
# 127.53.53.53:53 空闲,systemd-resolved 不会碰
```

| 地址 | 占用者 | NSC 能用吗 |
|------|-------|------------|
| `127.0.0.1:53` | (Linux/macOS 传统;某些 dnsmasq) | ⚠ 高冲突 |
| `127.0.0.53:53` | systemd-resolved | ❌ 必冲突 |
| `127.53.53.53:53` | 无 | ✅ **默认** |

### 权限提示

端口 < 1024 需 `CAP_NET_BIND_SERVICE`。Linux 下非 root 启动 NSC 时:

```bash
sudo setcap 'cap_net_bind_service=+ep' $(which nsc)
# 或 systemd unit 用 AmbientCapabilities=CAP_NET_BIND_SERVICE
```

若不想配 capability,改到非特权端口:

```bash
nsc --dns-listen 127.53.53.53:5353
```

但此时 `/etc/resolv.conf` 无法表达 port,需要借助 systemd-resolved 的 `DNSStubListenerExtra` 或 dnsmasq 做 `*.n.ns → 127.53.53.53:5353` 转发。

### 和 systemd-resolved 共存

由于默认地址完全不冲突,有两种推荐接法:

| 方案 | 做法 | 适用 |
|------|------|------|
| **A. 并列(默认)** | NSC 监听 `127.53.53.53:53`;把它**加到 `/etc/resolv.conf` 首行**,systemd-resolved 留给系统解析 | 本机只想让 `*.n.ns` 走 NSC,其余维持原状 |
| **B. 让 systemd-resolved 按域转发** | `resolvectl dns <iface> 127.53.53.53`<br/>`resolvectl domain <iface> '~n.ns'` | 想完全由 systemd-resolved 主导,NSC 做 `.n.ns` 域特化 resolver |

**不需要**停掉 systemd-resolved——这和旧的 `127.0.0.53:53` 默认最大的区别。

### 监听地址可配置

如果 `127.53.53.53:53` 也被占用(通常只在多 NSC 实例同宿主时发生),或你想在容器内用其它地址:

| 来源 | 形式 | 示例 |
|------|------|------|
| CLI 参数 | `--dns-listen <addr:port>` | `nsc --dns-listen 127.53.53.54:53` |
| 环境变量 | `NSC_DNS_LISTEN` | `NSC_DNS_LISTEN=127.53.53.53:5353` |
| 配置文件 | `[dns] listen = "..."` | 同上 |

优先级:CLI > 环境变量 > 配置文件 > 默认值。绑定失败时:

```
cannot bind DNS server on 127.53.53.53:53 — another resolver may be running
hint: check `ss -lntup | grep :53`, then use `--dns-listen <free-addr:port>`
      or switch to unprivileged port (e.g. 127.53.53.53:5353)
```
...
- 没有 systemd-resolved;mDNSResponder 不碰 `127.53.53.53`。
- **非 `127.0.0.1` 的环回地址需要手动 alias 到 `lo0`**:

  ```bash
  sudo ifconfig lo0 alias 127.53.53.53 up
  ```

  (可写进 LaunchDaemon 开机生效)
- 让 `*.n.ns` 特化解析走 NSC:`/etc/resolver/n.ns` 写一行 `nameserver 127.53.53.53`。
- 这两步 NSC 当前不自动做,由用户/部署脚本负责。
...
| `/etc/resolv.conf` 自动配置 | NSC 不改 | 需用户/部署脚本把监听地址(默认 `127.53.53.53`,或 `--dns-listen` 指定值)放到首位;或用 `resolvectl domain` 让 systemd-resolved 按域转发到 NSC |
