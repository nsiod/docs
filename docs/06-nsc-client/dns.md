...
```rust
pub const DNS_LISTEN_ADDR: &str = "127.0.0.53:53";   // crates/nsc/src/dns.rs:18
...
- 和 systemd-resolved 的 stub resolver 地址一致——多数 Linux 发行版默认的 `/etc/resolv.conf` 第一条就是它。用户若已运行 systemd-resolved，需先停掉再起 NSC；否则 `bind` 会失败：

  ```
  cannot bind DNS server — another resolver may be running on this address
  ```

  见 `crates/nsc/src/dns.rs:34`。

- `127.0.0.x` 是环回，无须 root。端口 53 是 DNS 标准端口，保证 `resolv.conf` 零改写。
- **缺点**：macOS 没有 systemd-resolved，需要通过 `/etc/resolver/` 的 per-domain 配置或网络配置指向 `127.0.0.53`。当前代码未处理这一点。
...
| `/etc/resolv.conf` 自动配置 | NSC 不改 | 需用户/部署脚本把 `127.0.0.53` 放到首位 |
