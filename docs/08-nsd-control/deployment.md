...
```mermaid
flowchart LR
    BUN[Bun Server<br/>:3000] --> HTTP[HTTP/SSE Routes]
    NOISE[Noise listener<br/>:4001<br/>Rust TLS proxy?] -->|relay| BUN
    QUIC[QUIC listener<br/>:4002<br/>spawn nsd-quic-proxy] -->|relay| BUN
    HTTP --> AUTH[auth.ts]
    HTTP --> REG[registry.ts]
    HTTP --> SSE[SSE stream handler]
```
...
graph TB
    ...
    subgraph Control["NSD 控制面（可水平扩展）"]
        NSD1[NSD instance 1]
        NSD2[NSD instance 2]
    end
