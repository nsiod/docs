...
[NSIO 四组件关系](./diagrams/ecosystem.d2)
...
[NSIO Workspace 分层结构](./diagrams/crate-architecture.d2)
...
| `acl_policy` | 取**并集**(带来源 NSD 标注) + **本地 `services.toml` ACL 保底** | 接入新 NSD 只扩不删规则,与 wg/proxy 同向;最终放行仍受站点主人本地 ACL 约束 |
| `services_report` | **广播给所有 NSD** | 每个 NSD 都要看到这台 NSN 声明的服务白名单 |
| `gateway_report` | **广播给所有 NSD** | 每个 NSD 都要知道网关健康状况 |

合并逻辑在 `crates/control/src/merge.rs`。NSN 对多 NSD 的态度是: "谁下发配置我都听,但最终是否放行由本地 `services.toml` ACL 兜底"。

[NSN 订阅多 NSD 的时序](./diagrams/multi-nsd-sequence.d2)

> 为什么 ACL 取并集 + 本地 ACL 保底?自托管 NSD 与云端 NSD 共存时,intersection 会让两边独有的规则互相消失(只有完全一致的条目才保留),运维界面看不出"规则到哪去了",极易误判。union 语义让每个 NSD 独立贡献规则,每条规则标注来源;即使某个 NSD 被攻破下发"放行全部",最终还要经过站点主人本地 `services.toml` ACL 的交集校验才能生效 —— 由站点主人在本地锁定安全边界。
...
[网关连接状态机](./diagrams/gateway-state-machine.d2)
