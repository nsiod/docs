...
3. 进入模块深入: [../02-control-plane/README.md](../02-control-plane/README.md) 控制面 → [../03-data-plane/README.md](../03-data-plane/README.md) 隧道 → [../05-proxy-acl/README.md](../05-proxy-acl/README.md) 代理与 ACL。

### 我是新入职工程师

1. **本文件** → [dns-naming.md](./dns-naming.md) —— 先把 `.ns` 命名和 `FQID` 搞清楚,看代码才不会迷路。
2. [data-flow.md](./data-flow.md) —— 跟着一个包走一遍,建立"解密→ACL→路由→代理"的直觉。
3. 直接挑一个你负责的 crate 读: [../07-nsn-node/README.md](../07-nsn-node/README.md)(入口) / [../04-network-stack/README.md](../04-network-stack/README.md)(smoltcp) / [../06-nsc-client/README.md](../06-nsc-client/README.md)(VIP/DNS)。

## 本目录文档索引

| 文档 | 内容 |
|------|------|
| [README.md](./README.md) | 本文件: 生态全景 + 读者路径 |
...
| 文档目录 | 对应 crate | 核心内容 |
|---------|-----------|---------|
| [02-control-plane/](../02-control-plane/README.md) | `common`, `control` | machinekey/peerkey 认证、SSE/Noise/QUIC 传输、MultiControlPlane 配置合并 |
| [03-data-plane/](../03-data-plane/README.md) | `tunnel-wg`, `tunnel-ws`, `connector` | gotatun UAPI、WsFrame 二进制协议、MultiGatewayManager 选路 |
| [04-network-stack/](../04-network-stack/README.md) | `netstack`, `nat` | smoltcp VirtualDevice、PacketNat DNAT/SNAT、HybridNatSend |
| [05-proxy-acl/](../05-proxy-acl/README.md) | `proxy`, `acl` | TCP/UDP 双向转发、HTTP Host 嗅探、TLS SNI 嗅探、仅允许 ACL 引擎 |
| [06-nsc-client/](../06-nsc-client/README.md) | `nsc` | 127.11.x.x VIP、本地 DNS 解析、NscRouter、HTTP 代理 |
| [07-nsn-node/](../07-nsn-node/README.md) | `nsn`, `telemetry` | 二进制入口、AppState、监控 HTTP API、OTel/Prometheus |
